...
def parse_interface(f):
  ...
  for line in f.splitlines():
    ...
    search_new2=re.search('^(Ethernet[1-8]/[0-9]+) is down',line)
    if search_new:
      INTERFACE_ID=search_new.group(1)
    if search_new2:
      INTERFACE_ID=search_new2.group(1)
    search=re.search('^(port-channel[0-9]+) is up',line)
    search2=re.search('^(port-channel[0-9]+) is down',line)
    if search:
      INTERFACE_ID=search.group(1)
    if search2:
      INTERFACE_ID=search2.group(1)
...
def Create_Excel_Table_xr(ip1,date1,data):
        ...
        filename='./nxos/session_logs/'+ip1+'_'+date1+'_xr_mcast.xlsx'
